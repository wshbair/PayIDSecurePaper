...
describe('E2E - adminApiRouter - PUT /users', function () {
    before(async function () {
        app = await helpers_1.appSetup();
    });
    it('Returns a 200 and updated user payload when updating an address', function (done) {
        ...
        request(app.adminApiExpress)
            ...
            .expect(http_status_1.default.OK, updatedInformation, done);
    });
    it('Returns a 200 and updated user payload when updating a PayID', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'alice$xpring.money';
        const updatedInformation = {
    ...
    it('Returns a 200 and updated user payload when removing all addresses', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
    ...
    it('Returns a 201 and inserted user payload for a Admin API PUT creating a new user', function (done) {
        ...
        request(app.adminApiExpress)
            ...
            .expect(http_status_1.default.Created, insertedInformation, done);
    });
    it('Returns a 201 and inserted user payload for a Admin API PUT creating a new user with an uppercase PayID provided', function (done) {
        // GIVEN a PayID known to not exist on the PayID service
        const payId = 'notjohndoe$xpring.money';
        ...
        request(app.adminApiExpress)
            ...
            .expect(http_status_1.default.Created, { ...insertedInformation, ...{ payId: newPayId } }, done);
    });
    it('Returns a 400 - Bad Request with an error payload for a request with a malformed PayID', function (done) {
    ...
    it('Returns a 400 - Bad Request with an error payload for a request with an existing PayID with multiple "$"', function (done) {
    ...
    it('Returns a 400 - Bad Request with an error payload for a request to update a PayID to a new value containing multiple "$"', function (done) {
        // GIVEN a PayID known to be in a bad format (missing $) and an expected error response payload
        const badPayId = 'alice$xpring.money';
        const expectedErrorResponse = {
            error: 'Bad Request',
    ...
    it('Returns a 409 - Conflict when attempting to update a user to a PayID that already exists', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        ...
        request(app.adminApiExpress)
            ...
            .expect(http_status_1.default.Conflict, expectedErrorResponse, done);
    });
    it('Returns a 409 - Conflict when attempting to PUT a new user to a PayID that already exists', function (done) {
    ...
    it('Returns a 415 - Unsupported Media Type when sending a wrong Content-Type header', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'alice$xpring.money';
      ...
      addresses: [
        ...
        const expectedErrorResponse = {
            ...
            message: "A 'Content-Type' header is required for this request: 'Content-Type: application/json'.",
        };
        ...
        request(app.adminApiExpress)
            ...
            .expect(http_status_1.default.UnsupportedMediaType, expectedErrorResponse, done);
    });
    it('Returns a 415 - Unsupported Media Type when Content-Type header is missing', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'alice$xpring.money';
...
//# sourceMappingURL=putUsers.test.js.map