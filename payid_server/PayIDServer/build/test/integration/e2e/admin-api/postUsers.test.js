...
describe('E2E - adminApiRouter - POST /users', function () {
    before(async function () {
        app = await helpers_1.appSetup();
    });
    it('Returns a 201 when creating a new user', function (done) {
        ...
        request(app.adminApiExpress)
            ...
            .expect(http_status_1.default.Created, done);
    });
    it('Returns a 201 when creating a new user with verifiedAddresses', function (done) {
        // GIVEN a user with a PayID known to not exist on the PayID service
        const userInformation = {
            ...
            verifiedAddresses: [
                {
                    ...
                    identityKeySignature: 'd2hhdCBhbSBJIGNvZGluZyB3aGF0IGlzIGxpZmUgcGxlYXNlIGhlbHA=',
                },
            ],
        };
        // WHEN we make a POST request to /users with that user information
        request(app.adminApiExpress)
            ...
            request(app.adminApiExpress)
                ...
                .expect(http_status_1.default.OK, userInformation, done);
        });
    });
    it('Returns a 201 when creating a new user with an identity key', function (done) {
    ...
    it('Returns a 201 when creating a new user, without an Accept-Patch header in the response', function (done) {
        // GIVEN a user with a PayID known to not exist on the PayID service
        const userInformation = {
        ...
        request(app.adminApiExpress)
            ...
            chai_1.expect(res.header).to.not.have.key('Accept-Patch');
            return done();
        });
    });
    it('Returns a 201 when creating a new user with an uppercase PayID', function (done) {
    ...
    it('Returns a 201 when creating a new user with an address without an environment (ACH)', function (done) {
        // GIVEN a user with a PayID known to not exist on the PayID service
        const userInformation = {
    ...
    it('Returns a 201 - creates PayID containing a "."', function (done) {
    ...
    it('Returns a 409 - Conflict when attempting to create a user that already exists', function (done) {
...
//# sourceMappingURL=postUsers.test.js.map