...
describe('E2E - adminApiRouter - GET /users', function () {
    ...
    it('Returns a 200 and correct information for a user known to exist', function (done) {
    ...
    it('Returns a 200 and correct information for a user known to have an identity key', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'verified$127.0.0.1';
        const expectedResponse = {
            payId: 'verified$127.0.0.1',
            identityKey: 'bGV0IG1lIHNlZSB0aGVtIGNvcmdpcyBOT1cgb3IgcGF5IHRoZSBwcmljZQ==',
            addresses: [],
            verifiedAddresses: [],
        };
        // WHEN we make a GET request to /users/ with that PayID as our user
    ...
    it('Returns a 200 and correct information for a user with an identity key and verified addresses', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'postmalone$127.0.0.1';
        const expectedResponse = {
            payId: 'postmalone$127.0.0.1',
            identityKey: 'aGkgbXkgbmFtZSBpcyBhdXN0aW4gYW5kIEkgYW0gdGVzdGluZyB0aGluZ3M=',
            addresses: [],
            verifiedAddresses: [
                {
                    paymentNetwork: 'BTC',
                    ...
                    identityKeySignature: 'TG9vayBhdCBtZSEgd29vIEknbSB0ZXN0aW5nIHRoaW5ncyBhbmQgdGhpcyBpcyBhIHNpZ25hdHVyZQ==',
                },
            ],
        };
        // WHEN we make a GET request to /users/ with that PayID as our user
    ...
    it('Returns a 200 and correct information but no identity key for a user known to have an empty string identity key', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
    ...
    it('Returns a 200 and correct information for a user known to exist without any addresses', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
    ...
    it('Returns a 200 and correct information for a user known to exist, when we use an uppercase PayID', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'ALICE$XPRING.MONEY';
        const expectedResponse = {
            payId: 'alice$xpring.money',
    ...
    it('Returns a 404 for an unknown PayID', function (done) {
        // GIVEN a PayID known to not exist on the PayID service
        const payId = 'johndoe$xpring.money';
        const expectedErrorResponse = {
            statusCode: 404,
            error: 'Not Found',
            message: 'No information could be found for the PayID johndoe$xpring.money.',
        };
        // WHEN we make a GET request to /users/ with that PayID as our user
        request(app.adminApiExpress)
            .get(`/users/${payId}`)
