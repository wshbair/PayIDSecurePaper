...
describe('E2E - adminApiRouter - PATCH /users/:payId', function () {
    before(async function () {
        app = await helpers_1.appSetup();
    });
    it('Returns a 201 when updating a user PayID', function (done) {
    ...
    it('Returns a 201 when updating a user PayID with an uppercase PayID', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
    ...
    it('Returns a 201 when updating a user PayID with the same PayID', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
    ...
    it('Returns a 404 - The original user PayID does not exist', function (done) {
    ...
    it('Returns a 415 - Unsupported Media Type when omitting the Content-Type header', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'alice$127.0.0.1';
        // AND a request to update that PayID to one known to already exist on the PayID Service
        const newPayId = {
        ...
        const expectedErrorResponse = {
            ...
            message: "A 'Content-Type' header is required for this request: 'Content-Type: application/merge-patch+json'.",
        };
        // WHEN we make a PATCH request to /users/:payId with the new PayID to update
        request(app.adminApiExpress)
            .patch(`/users/${payId}`)
    ...
    it('Returns a 415 - Unsupported Media Type when setting the Content-Type header to a wrong value', function (done) {
        // GIVEN a PayID known to resolve to an account on the PayID service
        const payId = 'alice$127.0.0.1';
...
//# sourceMappingURL=patchUsersPayId.test.js.map