...
const hostname = process.env.FABER_AGENT_HOST || 'localhost';
const port = 8021;
...
app.post('/creds/register-nym', async(req,res)=>{
    let faberAgent = new AgentService()
    const did = req.body.did
    const verkey = req.body.verkey
    const response = await faberAgent.RegisterNym({"did": did, "verkey": verkey})
    res.status(200).json({"result": response })
})
app.listen(serverPort, () => {
  console.log(`FABER API controller listening at http://localhost:${serverPort}`)
})
